<script>
  ...
  async function fetchAll() {
    try {
      const resp = await fetch("/api/allDocs");
  ...
  async function clearCompleted() {
    ...
    itemsToDelete.map((item) => save({ _deleted: true, ...item }));
    items = items.filter((item) => !item.completed);
  }

  function remove(index) {
    const itemToDelete = items[index];
    save({ _deleted: true, ...itemToDelete });
  ...
  async function createNew(event) {
    if (event.which === ENTER_KEY) {
      ...
      const { error } = await save(newItem);
      if (!error) {
  ...
  async function save(doc) {
    try {
      const resp = await fetch("/api/upsertDoc", {
        method: "POST",
        body: JSON.stringify({
          ...doc,
        }),
    ...
    {#if items.length > 0}
      <section class="main">
            ...
            >
              <div class="view">
                <input
                  ...
                  on:change={() => save(item)}
                />
                <label for={item.id} on:dblclick={() => (editing = index)}
        ...
        <footer class="footer">
          ...
          <ul class="filters">
            <li>
              <a class={currentFilter === "all" ? "selected" : ""} href="#/"
                >All</a
              >
            </li>
            <li>
...
<style>
  ...
  .clear-completed,
  .todoapp-body .clear-completed:active {
    float: right;
    position: relative;
    line-height: 20px;
    text-decoration: none;
